...
const Texture& assets::Manager::get_tile_texture(const data::id& id) {
    if (auto opt = try_get_tile_texture(id)) return opt.value();
    return inst()._placeholder_texture;
}
const Texture& assets::Manager::get_item_texture(const data::id& id) {
    if (auto opt = try_get_item_texture(id)) return opt.value();
    return inst()._placeholder_texture;
}

opt_cref<Texture> assets::Manager::try_get_tile_texture(const data::id& id) {
    if (inst()._tile_textures.contains(id)) return cref(inst()._tile_textures.at(id));
    return nullopt;
}
opt_cref<Texture> assets::Manager::try_get_item_texture(const data::id& id) {
