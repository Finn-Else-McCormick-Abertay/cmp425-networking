...
Chunk::Chunk(const Chunk& rhs) : _chunk_coords(rhs._chunk_coords), _neighbours(rhs._neighbours) {
    for (auto& [i, layer] : rhs._layers) {
        _layers[i] = layer;
        _layers[i]._owner = this;
        _layers[i]._layer = i;
    }
}

Chunk::Chunk(Chunk&& rhs) : _chunk_coords(move(rhs._chunk_coords)), _neighbours(move(rhs._neighbours)), _layers(move(rhs._layers)) {
    for (auto& [i, layer] : _layers) layer._owner = this;
}

Chunk& Chunk::operator=(const Chunk& rhs) {
    _chunk_coords = rhs._chunk_coords; _neighbours = rhs._neighbours;
    _layers.clear(); for (auto& [i, layer] : rhs._layers) {
        _layers[i] = layer;
        _layers[i]._owner = this;
        _layers[i]._layer = i;
    }
...
Chunk& Chunk::operator=(Chunk&& rhs) {
    _chunk_coords = move(rhs._chunk_coords); _neighbours = move(rhs._neighbours);
    _layers = move(rhs._layers);
    for (auto& [i, layer] : _layers) layer._owner = this;
    return *this;
}

Chunk::Layer::Layer(Chunk* owner, draw_layer layer) : _owner(owner), _layer(layer) {
    for (uint i = 0; i < TILE_COUNT; ++i) {
        _tiles[i] = "default::air"_id;
        _shapes[i] = TileShape::____;
    }
}
...
bool Chunk::has(draw_layer layer) const { return _layers.contains(layer); }

Chunk::Layer& Chunk::operator[](draw_layer layer) {
    if (!has(layer)) _layers[layer] = Layer(this, layer);
    return _layers.at(layer);
}
const Chunk::Layer& Chunk::operator[](draw_layer layer) const { return at(layer); }

Chunk::Layer& Chunk::at(draw_layer layer) { assert(has(layer)); return _layers.at(layer); }
const Chunk::Layer& Chunk::at(draw_layer layer) const { assert(has(layer)); return _layers.at(layer); }
...
bool Chunk::empty() const {
    for (auto& [_, layer] : _layers) { if (!layer.empty()) return false; }
    return true;
}

const id& Chunk::Layer::tile_at(const uvec2& pos) const { return is_in_range(pos) ? _tiles.at(index_from_coords(pos)) : nullid; }
TileShape Chunk::Layer::shape_at(const uvec2& pos) const { return _shapes.at(index_from_coords(pos)); }
bool Chunk::Layer::set_tile_at(const uvec2& pos, const id& tile) {
    ...
    //print<info, Chunk>("Chunk{}::{} set tile at {} to {}", _owner->_chunk_coords, _layer, pos, tile);
    ...
    for (int i = -1; i <= 1; ++i) { for (int j = -1; j <= 1; ++j) {
        ...
        if (bounds_signs.x == 0 && bounds_signs.y == 0) update_shape_of(nbr_pos);
        else if (_owner) {
            auto neighbour_chunk = _owner->get_chunk_neighbour(bounds_signs);
            if (neighbour_chunk && neighbour_chunk->has(_layer)) {
                ivec2 other_chunk_nbr_pos = nbr_pos + bounds_signs * -(int)SIZE_TILES;
                neighbour_chunk->at(_layer).update_shape_of(other_chunk_nbr_pos);
            }
        }
    }}
    return true;
...
void Chunk::update_all_layer_shapes_at(const uvec2& pos) {
    for (auto& [_, layer] : _layers) layer.update_shape_of(pos);
}
...
void Chunk::update_shapes_along_neighbour_border(const ivec2& dir, bool neighbour_only) {
    ...
    if (dir.x != 0 && dir.y != 0) {
        ...
        if (neighbour) neighbour->update_all_layer_shapes_at(opposing_corner);
    }
    // Vertical
    else if (dir.y == 0) {
        ...
        for (uint i = 0; i < SIZE_TILES; ++i) {
            ...
            if (neighbour) neighbour->update_all_layer_shapes_at(ivec2(opposing_column, i));
        }
    }
    // Horizontal
    else if (dir.x == 0) {
        ...
        for (uint i = 0; i < SIZE_TILES; ++i) {
            ...
            if (neighbour) neighbour->update_all_layer_shapes_at(ivec2(i, opposing_row));
        }
    }
}

bitset<8> Chunk::Layer::find_connections_at(const uvec2& pos) const {
    assert(is_in_range(pos));
    ...
    for (int i = -1; i <= 1; ++i) { for (int j = -1; j <= 1; ++j) {
        ...
        if (bounds_signs.x == 0 && bounds_signs.y == 0) connects = should_connect(tile_id, tile_at(nbr_pos));
            ...
            if (neighbour_chunk && neighbour_chunk->has(_layer)) {
                ivec2 other_chunk_nbr_pos = nbr_pos + bounds_signs * -(int)SIZE_TILES;
                connects = should_connect(tile_id, neighbour_chunk->at(_layer).tile_at(other_chunk_nbr_pos));
            }
        }
...
void Chunk::Layer::update_shape_of(const uvec2& pos) {
    assert(is_in_range(pos));
    if (auto opt = data::Manager::get_tile(tile_at(pos))) {
    ...
    _shapes[index_from_coords(pos)] = tile_impl::from_connections(find_connections_at(pos));
}